...
| NFR-5 (Reliability under poor connectivity / infrastructure failure) | The app tolerates dropped connections and stale devices, and degrades gracefully — never crashes or hangs — if Redis itself becomes unavailable | Presence self-heals via a Redis-backed TTL/heartbeat sweep (`redis/presence.ts`, TD-05); idempotent state transitions; polling fallback alongside sockets. A real production incident (Technical_Debt_Plan.md TD-05 update, Testing_Report.md D-24–D-26) found this needed strengthening: `GET /collectors/nearby`/`GET /pins/nearby` now fall back to Postgres geo queries, presence writes always durably persist to Postgres regardless of Redis's outcome, rate limiting degrades to an in-memory counter instead of failing wide open, every Redis/BullMQ call in a live request path is time-boxed (`utils/withTimeout.ts`), and a top-level `uncaughtException`/`unhandledRejection` handler (`index.ts`) is a last-resort backstop against ioredis's own promise-bypassing failure modes |
