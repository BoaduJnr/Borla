...
All fifteen were caught by testing immediately after (or, for D-05/D-06/D-07/D-08/D-10/D-12/D-14/
D-15, well after) the corresponding feature — eight by the automated suite (D-11 by reasoning
through the code while building an unrelated feature, D-13 by directly observing stuck
production queue items while manually verifying D-12), seven by deliberately exercising the
live deployed app or reading its logs (D-05 by me re-testing production myself; D-07, D-10,
D-12, D-14, and D-15 reported back by the user; D-08 surfaced in production logs once the
Gemini key went live), and D-09 by a scripted screenshot pass rather than by reading the code —
...
Some defects are only found by putting the actual feature in front of the actual person it's
for, and some are only found by watching your own fix operate for real.
